...
def index():
    ...
    stats = {
        ...
        'rev_table' : df2.groupby(by = ['Category','App']).sum().sort_values('Reviews',ascending = False).head(10).reset_index()[['Category','App','Reviews','Rating']].to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    genres = playstore.pivot_table(index = 'Genres',values = 'Installs',aggfunc = 'sum').sort_values('Installs',ascending = False).head(10)
    genres.plot.barh()
